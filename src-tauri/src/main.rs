                ...
                if let Ok(file) = fs::File::open(file_path.clone()) {
                    ...
                    match serde_json::from_reader::<_, MaFile>(&file){
                        Ok(ma_file) => {
                            ...
                            if ma_file.Session.AccessToken.is_some() && ma_file.Session.RefreshToken.is_some() {
                                tokens = Some(steamguard::token::Tokens::new(ma_file.Session.AccessToken.unwrap(), ma_file.Session.RefreshToken.unwrap()))
                            } else {
                                tokens = None
                            }
                            ...
                            let account = steamguard::SteamGuardAccount{
                                ...
                                revocation_code: steamguard::SecretString::new(ma_file.revocation_code),
                                shared_secret: steamguard::token::TwoFactorSecret::parse_shared_secret(ma_file.shared_secret).unwrap(),
                                token_gid: ma_file.token_gid,
                                identity_secret: steamguard::SecretString::new(ma_file.identity_secret),
...
fn get_code(state: State<AppState>, name: &str) -> String {
    return state.accounts.lock().unwrap().get(name).unwrap().generate_code(std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs());
}
...
#[tauri::command]
fn get_confirmations(state: State<AppState>, name: &str, refresh: bool) -> Result<Vec<ConfirmationInfo>, String> {
